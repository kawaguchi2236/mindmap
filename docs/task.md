...
- （2026-09-15・ユーザー指示）**エディタとテンプレート画面をハンドオフのデザインに寄せた。**
  それまでのエディタは枠付きカードのノードだったが、`design/ハンドオフ.md` `#2b`（Press）は
  枠なしの文字組みで、階層ごとに書体の大きさと濃度を変える案。合わせて以下を変えた。
  - ノードの大きさが**階層依存**になった。`src/features/editor/layout.ts` の
    `NODE_WIDTH` / `NODE_HEIGHT` / `H_GAP` は廃止し、`tierOf(depth)` /
    `nodeBox(text, depth)` / `depthToX(depth)` / `columnStep(parentDepth)` に置き換えた。
    レイアウト系の関数はすべて `buildDepthIndex()`（`tree.ts`）で深さを配ってから計算する。
  - エディタは**ヘッダーバーを持たない**。マップ名・保存状態・Undo/Redo・PNG・設定への導線・
    ショートカットのヒント・倍率は、すべて React Flow の `<Panel>` で四隅に浮かべている
    （Panel は `.react-flow__viewport` の外なので PNG 書き出しに写らない）。
  - 選択中のノードには `<NodeToolbar>` のコンテキストツールバーが出る。常設ツールバーは無い。
  - **既存のマップは古い座標のまま**（間隔が旧 244px 基準）。ドラッグで置いた位置を勝手に
    動かさない方針（CLAUDE.md §6・§9）なので、開いた瞬間の再整列はしていない。
    右下の「整列」を押せば新しい間隔になる。ただし**重なりだけは開いた時点で解消する**
    （2026-09-15 追記。`createInitialState` が `resolveOverlaps` を通す）。古い座標のまま
    被って見えるのは不具合であって手配置ではないため。重なった相手を下げるだけなので、
    手で並べた配置そのものは残る。
  - テンプレート画面 `/templates` を新設（`docs/要件定義.md` SCR-005・`#3c`）。
    選ぶと IndexedDB にマップを作ってエディタへ入る。ゲストでも使える。
...
- [x] アイコン（自前 SVG 16種。依存追加なし。2026-09-15 に minus / text / collapse を追加）

## 3. レイアウト — 担当 A
- [x] Header（`actions` スロットあり。テーマ切替はヘッダーが常に持つ。
      **エディタはヘッダーを使わない**（`#2b` はヘッダーバーを持たない案）。一覧・設定・テンプレートで使う）
- [x] Sidebar（マップ一覧で使用。エディタでは出さない）
- [x] Main Layout（`variant="editor"` は 100dvh 固定でキャンバスを全面に使う。
      既定の variant は縦 flex で、最下部の広告帯を画面下へ逃がす）
...
- [ ] レスポンシブ対応（PC ファースト。**狭い画面での目視は未実施**）
      ...
      - **`src/features/**` にはメディアクエリが1つも無い**（マップ一覧・設定・同期・書き出し）
        → 2026-09-15 に `editor.css`（760px でクロームを寄せる／ヒントを隠す）と
          `TemplateScreen.module.css`（1080 / 760 / 480px で列数を落とす）を追加した
...
- [x] ドラッグ中表示
- [x] Drop処理
- [x] **ドロップ後の重なり解消**（2026-09-15・ユーザー指示）。落とした場所はユーザーが指した
  場所なので動かしたノード自身は固定し、被った相手のほうを下げる
  （`resolveOverlaps(nodes, pinnedId)`）。以前は「ドラッグでは整列しない」方針で重なりを
  許していたが、被ったまま残るのは `docs/要件定義.md` FR-NODE-009（交差や重なりを極力抑える）
  に反する。
...
## 14. 広告 — 担当 B
- [x] マップ一覧
- [x] テンプレート（テンプレート画面を追加したので設置先ができた。`slot="templates"`）
- [x] 設定画面
- [x] Free / Pro判定（Phase 1 は課金基盤なしのため全員 Free。関数1つのみ）

## 14.5 テンプレート画面 — （2026-09-15 追加）

`docs/要件定義.md` SCR-005 は「Phase 1 では画面だけ先行してもよい／機能自体は Should」。
デザイン（`#3c`）に合わせて画面を作り、選んだら実際にマップができるところまで入れた。

- [x] 画面（`/templates`。ヘッダーのナビと、一覧・設定のサイドバーから導線あり）
- [x] テンプレート8種＋「空のマップ」（`src/features/templates/catalog.ts`）
- [x] 分類での絞り込み（ハンドオフのタブに「すべて」を足して既定にしている）
- [x] 選ぶとローカルにマップを作ってエディタへ遷移
- [x] 広告帯（`slot="templates"`）
- [ ] 実機での目視（Chrome のライト／ダークのみ確認済み。Edge / Safari は未確認）
